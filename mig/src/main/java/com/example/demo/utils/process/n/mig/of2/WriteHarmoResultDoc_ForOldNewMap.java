...
import com.example.demo.utils.process.n.mig.Abraic2Chinese;
import com.example.demo.utils.process.n.mig.ProcessFileReader;
import com.example.demo.utils.process.n.mig.X;

public class WriteHarmoResultDoc_ForOldNewMap {
    ...
    public WriteHarmoResultDoc_ForOldNewMap() {
    }

    public WriteHarmoResultDoc_ForOldNewMap(String file) {
        this.file = file;
    }
    ...
    public static void outOneTable(String msgName, String title, ArrayList ar) {
        try {
            ...
            int numRows = ar.size() + 2; // 資料列 + 2列標題
            XWPFTable table = document.createTable(numRows, 16);
            
            CTTblWidth tblWidth = table.getCTTbl().addNewTblPr().addNewTblW();
            tblWidth.setType(STTblWidth.PCT);
            tblWidth.setW(BigInteger.valueOf(5000));

            int[] widths = {3, 3, 3, 3, 10, 10, 6, 3, 9, 10, 5, 10, 10, 6, 3, 6};
            ...
            mergeCellVertically(table, 0, 1, 10);
            setCellText(row0.getCell(10), "備註", 12, true, true, "FFD700");
            colIdx++;
            ...
            mergeCellHorizontally(row0, 11, 15);
            setCellText(row0.getCell(11), oldDataTitle, 12, true, true, "FFD700");
            ...
            int[] singleCols = {4, 5, 6, 7, 8, 11, 12, 13, 14, 15};
            ...
            for (int i = 0; i < ar.size(); i++) {
                ...
                if (mmm != null) {
                    if (tblNum % 3 == 1) {
                        remark = mmm.getOldNewMapRemark(tblNum, item.newChnName);
                    } else if (tblNum % 3 == 2) {
                        remark = mmm.getOldNewMapRemark(tblNum, item.newChnName);
                    } else if (tblNum % 3 == 0) {
                        remark = mmm.getOldNewMapRemark(tblNum, item.oldChnName);
                    }
                }
                item.oldNewMapRemark = ""; 
                setCellText(row.getCell(col++), item.oldNewMapRemark, 12, false, false, null);
    ...
    public static void run(M mmm) {
        WriteHarmoResultDoc_ForOldNewMap.mmm = mmm;
